...
@Component({
  selector: 'bu-accordion',
  template:`
  <ng-content></ng-content>
          `,
  styleUrls:['./app-business-unit-accordion.component.scss'],
  host: {
    'class': 'panel-group'
  }
})
export class BuAccordion {
  @Input() oneAtaTime:boolean = true;
  groups: Array<BuAccordionGroup> = [];

  addGroup(group: BuAccordionGroup): void {
    this.groups.push(group);
  }

  closeOthers(openGroup: BuAccordionGroup): void {
    if(this.oneAtaTime){
      this.groups.forEach((group: BuAccordionGroup) => {
        if (group !== openGroup) {
          group.isOpen = false;
        }
      });
  ...
  removeGroup(group: BuAccordionGroup): void {
                    ...
                    <h4 class="panel-title">
                      ...
                      <a tabindex="0">
                        ...
                        <div class='panel-buttons align-right' style="display: inline-block;">
                          <button class='btn btn-sm btn-darkgray' (click)='delete()'>Delete</button>
                        </div>
                      </a>
                    </h4>
                  </div>
...
export class BuAccordionGroup implements OnDestroy, OnInit {
  ...
  constructor(private accordion: BuAccordion, private _sanitizer: DomSanitizer) {
...
export const APP_BU_ACCORDION = [BuAccordion , BuAccordionGroup];
